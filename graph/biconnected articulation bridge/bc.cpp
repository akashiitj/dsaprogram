...
void dfsap(int svn)//source vertex no
{
    ...
    while(hp!=NULL)
    {
        ...
        if(visit[cvn]!=1)
        {
            child++;
            vidfs[cvn].pi=svn;
            bc.pb(mp(svn,cvn));//for biconnected component
            dfsap(cvn);
            ...
            {
                ap.push_back(svn);
                while(bc.back()!=mp(svn,cvn))
                {
                    cout<<bc.back().first<<" "<<bc.back().second<<"     ";
                    bc.pop_back();
                }
                cout<<bc.back().first<<" "<<bc.back().second<<"     ";
                bc.pop_back();
        ...
        {
            //bc.pb(mp(svn,cvn));//dont do this
            if(vidfs[svn].minbe>vidfs[cvn].intime1)
            {
                bc.pb(mp(svn,cvn));//you forgot this, if they are in same cc then only you have to push them
...
main()
{
    ...
    cout<<"\nenter 2 vertex no in which you wanted to add edge and type -1 to end"<<endl;
    while(true)
    {
        cin>>v1;
        if(v1==-1)break;
