...
int cnt(int i,int j,int sti,int si,int ei)
{
    ...
    if(lazy[sti]!=0)
    {
        ...
        if(si!=ei)
        {
            lazy[sti*2]=(lazy[sti]+lazy[sti*2])%2;
            lazy[sti*2+1]=(lazy[sti]+lazy[sti*2+1])%2;
        }
        lazy[sti]=0;
    }
    if(i<=si&&j>=ei)return st[1][sti];
...
void update(int i,int j,int dx,int sti,int si,int ei)//dx is always 1 so no need to include dx
{
    ...
    {   int x=st[0][sti];
        ...
        lazy[sti*2]=(dx+lazy[sti*2])%2;
        lazy[sti*2+1]=(dx+lazy[sti*2+1])%2;
        return;
    }
    int mid=(si+ei)/2;
